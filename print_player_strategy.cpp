...
static void print_strategy_r( PureCfrPlayer &pcp,
			      State &state,
			      ...
			      const int max_round )
{
  ...
  if( p == currentPlayer( ag->game, &state ) ) {
    ...
    for( int bucket = 0; bucket < num_buckets; ++bucket ) {
      ...
      pcp.get_action_probs( state, action_probs, bucket );
  ...
  for( int a = 0; a < num_choices; ++a ) {
    ...
    print_strategy_r( pcp, new_state, ag, p, max_round );
  }
}
...
int main( const int argc, const char *argv[] )
{
  ...
  int index = 1;
  PureCfrPlayer pcp( argv[ index ] );
  ++index;
  const AbstractGame *ag = pcp.get_abstract_game( );
  ...
  for( int p = 0; p < ag->game->numPlayers; ++p ) {
    ...
    printf( "=== PLAYER %d ===\n", p + 1 );
    print_strategy_r( pcp, state, ag, p, max_round );
  }
